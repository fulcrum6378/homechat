...
public class AsyncPortscan extends AsyncTask<Void, Object, Void> {
    ...
    private static final long TIMEOUT_RW = 3L * 1000 * 1000000; // ns
    ...
    private final ByteBuffer byteBuffer = ByteBuffer.allocate(MAX_READ);
    ...
    protected AsyncPortscan(Activity activity, String host, int _rate) {
        ipAddr = host;
        TIMEOUT_CONNECT = _rate * 1000000L; // ms to ns
                    ...
                    synchronized (selector.selectedKeys()) {
                        ...
                        while (iterator.hasNext()) {
                            ...
                            try {
                                ...
                                if (Objects.equals(e.getMessage(), E_REFUSED)) {
    ...
    private void closeSelector() {
        try {
            if (selector.isOpen()) synchronized (selector.keys()) {
                for (SelectionKey selectionKey : selector.keys())
                    finishKey(selectionKey, FILTERED);
                selector.close();
            }
        } catch (IOException e) {
            // TODO MAHDI Log.e(TAG, e.getMessage());
